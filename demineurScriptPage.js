...
{   
	...
	/****************************  Fin zone détection clic souris ************************/
     ...
     // le jeu est redessiné toutes les 50 ms
	  ...
	  var myInterval = setInterval(evoluer, 50); // Redessine le canvas
	  var chrono = setInterval(timer,1000)		 // Appele la fonction timer
	  var finGame = setInterval(checkEnd,1000)   // Check la fin du jeu
...
window.onload = function(){
  backgroundMusic.volume = 0.5; // Mets le son a 50 %
  //document.getElementById("partieWin").value =null; 	
  
 /* function titreGlissant(){
	  var titre = "Ceci est un démineur !";
	  var tablTitre = new Array();
	  var tablTitreDecal = new Array();
	  for(i=0;i<titre.length;i++){tablTitre[i]=titre.charAt(i);}
	  for(j=0;j<titre.length;j++){if(j+1==titre.length+1){tablTitreDecal[0]=tablTitre[j];tablTitreDecal[j+1]=" "}tablTitreDecal[j]=tablTitre[j+1];}
	  for(k=0;k<tablTitre.length;k++){titre="";titre= titre + tablTitreDecal[k];console.log(titre);}
	  //console.log(tablTitreDecal);
	  document.title = titre;
  }
  var interTitreGlissant = setInterval(titreGlissant,500);*/
}
